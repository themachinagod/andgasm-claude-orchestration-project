...
| PR review enforcement is pipeline-process-based, not GitHub-approval-based | All agents run under the same GitHub account. GitHub's "require N approvers" can't distinguish PR author from reviewer. Branch protection enforces CI status checks only. Review rigour enforced by pipeline review cycle (specialist reviewers, PR comments, concern routing, coordinator gate). No PR is merged without full review — non-negotiable. |
| Merge authority is role-based, not account-based | Coordinator is the merge authority. Spec-compliance is the final gate for implementation PRs. These are process roles, not separate GitHub users. |
