...
- Read the existing codebase in the relevant Node/TypeScript repos (paths from repos.yaml)
...
8. Run CI locally — `tsc --noEmit && npm test && npm run lint`. All must pass.
...
3. Review for:
   ...
   - **Stack patterns**: does it follow established TypeScript conventions for
     this codebase? (strict type usage, discriminated unions, Zod runtime
     validation at boundaries, typed error hierarchy, vitest/jest patterns)
